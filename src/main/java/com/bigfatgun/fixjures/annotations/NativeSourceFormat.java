...
public enum NativeSourceFormat implements SourceFormat {
	...
	Json {
		 ...
		 */
		@Override
		public SourceFactory createSourceFactory(final Fixture fixtureAnnotation) {
			return new SourceFactory() {
				public FixtureSource newInstance(final Class<?> type, final String name) {
					try {
						return JSONSource.newJsonStream(fixtureAnnotation.type().openStream(fixtureAnnotation.value()));
					} catch (IOException e) {
	 ...
	 * {@inheritDoc}
	 */
	public abstract SourceFactory createSourceFactory(final Fixture fixtureAnnotation);
}

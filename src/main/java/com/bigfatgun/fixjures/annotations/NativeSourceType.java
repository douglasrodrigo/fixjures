package com.bigfatgun.fixjures.annotations;

import java.io.ByteArrayInputStream;
import java.io.FileNotFoundException;
import java.io.IOException;
import java.io.RandomAccessFile;
import java.nio.channels.Channels;
import java.nio.channels.ReadableByteChannel;

import com.bigfatgun.fixjures.FixtureSource;

/**
 * An enumeration of source types.
 *
 * @author Steve Reed
 */
public enum NativeSourceType implements SourceType {

	/**
	 * File.
	 */
	File {
		/**
		 * Opens a stream of a file with the given name.
		 *
		 * @param filename file name
		 * @return file bytes
		 * @throws FileNotFoundException if the file is not found
		 */
		@Override
		public ReadableByteChannel openStream(final String filename) throws FileNotFoundException {
			return new RandomAccessFile(filename, "r").getChannel();
		}
	},

	/**
	 * String literal.
	 */
	Literal {
		/**
		 * Returns the string literal as a byte stream.
		 *
		 * @param literal string
		 * @return string bytes
		 */
		@Override
		public ReadableByteChannel openStream(final String literal) {
			return Channels.newChannel(new ByteArrayInputStream(FixtureSource.getBytes(literal)));
		}
	};

	/**
	 * {@inheritDoc}
	 */
	@Override
	public abstract ReadableByteChannel openStream(final String literal) throws IOException;
}

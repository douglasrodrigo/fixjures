		...
		for (final Method m : cls.getMethods()) {
         if (m.isAnnotationPresent(Fixture.class)) {
				...
				if (paramTypes.length == 1 && m.getReturnType() == Void.TYPE) {
					...
					m.invoke(obj, newFactory(fixture).createFixture(paramTypes[0], fixture.name()));
				}
			}
      }
	}

	private static FixjureFactory newFactory(final Fixture fixtureAnnotation) {
		final NativeSourceFormat fmt = fixtureAnnotation.format();
		final SourceFactory fact = fmt.createSourceFactory(fixtureAnnotation);
